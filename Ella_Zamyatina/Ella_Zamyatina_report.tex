...
\begin{itemize}
    \item Реализована функция \verb|apply_convolution| для двумерной свёртки изображений, работающая с цветными (RGB) и полутоновыми изображениями, с четырьмя режимами дополнения границ.
    \item Подготовлен набор из восьми стандартных ядер свёртки размером $3 \times 3$ и $5 \times 5$.
    \item Проведена оптимизация: замена тройного цикла на \verb|sliding_window_view| и \verb|np.einsum| дала ускорение более чем в 350 раз.
